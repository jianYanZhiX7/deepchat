...
name: agent-reach
description: >
  MUST USE when the user wants to 调研/research/搜索/search/查/找/look up anything
  on the internet — e.g. 全网调研 X / 帮我调研一下 X / 查一下 X / 搜搜 X /
  看看大家怎么评价 X / X 上有什么讨论 / research this topic。

  Also MUST USE when the user mentions any platform or shares any URL/链接:
...
本 skill 随 DeepChat 分发，内容取自上游 Agent Reach（MIT）。上游会持续更换失效的接入方式，
DeepChat 会随版本更新这份文档；不要执行上游的自更新提示。

## 首次使用：安装 Agent Reach

零配置通道（`curl https://r.jina.ai/URL`、`gh`、`yt-dlp`）无需安装。其余平台需要
`agent-reach` CLI 及其上游工具。

1. 先用 `exec` 检查是否已安装：`agent-reach --version`。
2. 未安装时，先向用户说明将要安装的内容并取得授权，再执行下面的命令；未经授权不要安装系统包。
3. 安装到独立位置，**绝不写入用户工作目录**：

```bash
# 推荐 pipx（隔离且自动进 PATH）
pipx install https://github.com/Panniantong/agent-reach/archive/main.zip

# 没有 pipx 时用 venv（PEP 668 / 系统 Python 场景）
python3 -m venv ~/.agent-reach-venv
~/.agent-reach-venv/bin/pip install https://github.com/Panniantong/agent-reach/archive/main.zip
# 之后用 ~/.agent-reach-venv/bin/agent-reach 调用
...
若 `agent-reach` 不在 PATH，先用 `command -v agent-reach` 定位；仍找不到时，用上面 venv 的绝对路径调用。
...
5. **凭据只用小号**：需要 Cookie 或浏览器登录态的平台存在封号风险，提醒用户使用专用小号。
...
```bash
# Exa 网页搜索（需要 mcporter 与 Exa MCP；见 references/search.md）
mcporter call exa.web_search_exa query="query" numResults=5

# 通用网页阅读（无需安装）
curl -s "https://r.jina.ai/URL"

# GitHub 搜索（需要 gh CLI 且已登录）
gh search repos "query" --sort stars --limit 10

# YouTube 字幕（需要 yt-dlp；注意：B站不要用 yt-dlp，失败重试链见 video.md）
yt-dlp --write-sub --write-auto-sub --skip-download -o "/tmp/%(id)s" "URL"

# V2EX 热门（无需安装）
curl -s "https://www.v2ex.com/api/topics/hot.json" -H "User-Agent: agent-reach/1.0"

# B站搜索（需要 bili-cli，无需登录）
