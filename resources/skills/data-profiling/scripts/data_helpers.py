...
_REPO_ROOT = Path.cwd()  # Cowork: resolve .knowledge/ in the working folder
_env_loaded = False


def _load_env():
    """No-op in the Cowork plugin: credential files are not used here.

    Warehouse credentials live in Cowork connectors (set up in Customize), never
    in files inside the working folder. Kept as a stub so callers stay unchanged.
    """
    global _env_loaded
    _env_loaded = True
    return
...
def detect_active_source():
    ...
    # Fallback: manifests written per connect-data's spec carry a `connection:` block
    # instead of `local_data`. Treat a csv-type connection as the working folder.
    if not local_data:
        conn_blk = manifest.get("connection", {}) or {}
        if conn_blk.get("type") in ("csv", "excel", "files", "local"):
            local_data = {"path": conn_blk.get("path", ".")}
    ...
    # path stays on the local DuckDB file (unchanged behaviour for local files).
