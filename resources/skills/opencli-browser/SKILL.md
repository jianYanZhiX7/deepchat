...
This skill is for **driving a live browser** to accomplish an agent task.
...
If `browser open` or `browser analyze` returns `sitemap.available: true`, prefer it as prior context for pages, actions, workflows, APIs, and pitfalls before continuing a multi-step site flow. The sitemap is not truth: if the browser state disagrees, trust the browser.
...
- `opencli-usage` — the top-level map of adapters, flags, and output formats.
- `smart-search` — routing a search or research request to the right adapter.
