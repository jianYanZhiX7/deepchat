...
| 用途 | 规范路径 |
|---|---|
| 通用质量清单 | `references/review-quality.md` |
| 通用内容评分 rubric | `references/quality-rubric.md` |
| 去 AI 味方法 | `references/anti-ai-writing.md` |
| 剧情循环/高潮公式 | `references/plot-core-methods.md` |
| 角色关系/好感度 | `references/character-relations.md` |
| 对话质量 | `references/dialogue-mastery.md` |
| 审查禁用词 | `references/banned-words.md` |
| 平台 rubric | `references/rubrics/{fanqie,qidian,zhihu}.md` |
| 标点预检脚本 | `scripts/normalize-punctuation.js` |
| AI句式预检脚本 | `scripts/check-ai-patterns.js` |
| 作者习惯协议 | `references/author-memory.md` |
| 作者习惯事务脚本 | `scripts/author_memory_commit.py` |
...
full/lean 模式下，主会话必须把“审查基准包摘要”直接写进每个 Agent prompt。**不要要求子 Agent 必须读取 `references/*` 才能完成任务**；如需补充，只读取本 Skill 的 `references/*`，最终遵守注入的 rubric 摘要和统一 Findings Schema。

### 跨批审查落盘契约（所有模式）

只要多章/整卷/整本审查被拆成两批及以上，full、lean、solo 都维护 **{项目根}/.state.md**：
...
**.** 只保存审查状态，不属于小说事实追踪；不得借此修改正文、设定、大纲或 `追踪/`。
...
4. **识别目标平台并加载 rubric**：
   ...
   - 番茄小说 → 优先读取 `references/rubrics/fanqie.md`；不可读时使用内置番茄 fallback 摘要。
   - 起点 → 优先读取 `references/rubrics/qidian.md`；不可读时使用内置起点 fallback 摘要。
   - 知乎盐言 → 优先读取 `references/rubrics/zhihu.md`；不可读时使用内置知乎 fallback 摘要。
   - 未识别平台 → 优先读取 `references/quality-rubric.md`；不可读时使用内置通用网文内容 rubric，并报告 `Rubric: generic web-fiction` 与 `Rubric Source: file | embedded fallback`。
...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - 这三个预检脚本只读；`story-review` **不修改正文、设定或大纲文件**，需要自动修复正文时建议转 `/story-deslop`。full / lean 模式只有下方「追踪文件维护」允许修改 `追踪/`；分批审查的所有模式都可按上方契约写 **.state.md**，solo 除该状态外不写项目内容。
...
- 提示指令：
  ...
  可选补充参考：本 Skill 的 `references/review-quality.md`、`references/plot-core-methods.md`；若不可读，不影响审查。
  检查项：
  1. 这一章是否推进了故事主题？
  2. 大纲结构是否完整（钩子/爽点/悬念）？
  3. 情绪节奏是否合理？
...
- 提示指令：
  ...
  Rubric Source: file | embedded fallback
  相关角色文件：{角色设定文件路径}
  可选补充参考：本 Skill 的 `references/character-relations.md`、`references/dialogue-mastery.md`；若不可读，不影响审查。
  检查项：
  1. 角色语言风格是否与语言风格档案一致？
  2. 对话是否千篇一律或信息过满？
  3. 人物弧线是否连贯？
  ...
  7. 好感度进度是否可感知？
  8. 对话三症状（可选读 `references/dialogue-mastery.md` 自查项）：① 机械对话/问答式/句间无情绪承接；② 角色当「科普嘴」整段讲设定原理(Gate G 同样管台词)；③ 说话不分场合(高压/生死 beat 的玩笑、口头梗、插科打诨出戏)。命中按 S2/S3 报具体引用+改法。
...
- 提示指令：
  ...
  可选补充参考：本 Skill 的 `references/anti-ai-writing.md`、`references/banned-words.md`、`references/review-quality.md`；若不可读，不影响审查。
  检查项：
...
- 提示指令：
  ...
  审查基准包摘要：{Phase 1 形成的 rubric / fallback 摘要，必须内联}
  Rubric Source: file | embedded fallback
  可选补充参考：本 Skill 的 `references/review-quality.md`；若不可读，不影响事实冲突扫描。
  检查项：
  1. 角色属性是否前后一致？
  2. 世界规则是否被违反？
...
不 spawn Agent。先按 Phase 1 第 4 步识别目标平台并加载对应 rubric；即使是 solo，也必须用平台 rubric、`references/quality-rubric.md` 或内置审查基准包校准判断。

solo 必须执行基础检查：
1. 格式合规性检查（戏剧单元/画面分段、无机械字数切分、无空行、对话格式、主语/角色名节奏）。
2. 简单的设定一致性 grep（角色名、属性、关键设定、伏笔关键词）+ 推理型一致性检查（规则边界、设定层级、跨章因果链、可滥用漏洞、代价一致性）。
3. AI 味与禁用词检查（优先读取 `references/banned-words.md` 与 `references/anti-ai-writing.md`，不可读时使用内置 AI 味 / 禁用词 fallback 速查）。
4. 通用网文内容评分（优先读取 `references/quality-rubric.md`，不可读时使用内置通用网文内容 rubric）。
